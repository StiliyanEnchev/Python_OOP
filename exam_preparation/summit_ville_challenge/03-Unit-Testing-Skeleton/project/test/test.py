...
class TestClimbingRobot(TestCase):
    ...
    def setUp(self):
        ...
        self.robot_with_softwares = ClimbingRobot(
            'Indoor',
                                                  "Type_part",
                                                  100,
                                                  50
    ...
    def test_get_available_capacity_returns_the_correct_sum(self):
        expected_result = self.robot_with_softwares.get_available_capacity()
        result = self.robot_with_softwares.capacity - sum(s['capacity_consumption'] for s in self.robot_with_softwares.installed_software)
    ...
    def test_get_available_memory_returns_the_correct_sum(self):
        expected_result = self.robot_with_softwares.get_available_memory()
        result = self.robot_with_softwares.memory - sum(s['memory_consumption'] for s in self.robot_with_softwares.installed_software)

        self.assertEqual(result, expected_result)

    def test_install_software_with_one_requirement_higher_return_error_message(self):
        result = self.chucky.install_software(
            {'name': 'PyCharm', 'memory_consumption': 50, 'capacity_consumption': 400})
    ...
    def test_install_software_with_max_capacity_taken(self):
        result = self.chucky.install_software(
            {'name': 'PyCharm', 'memory_consumption': 50, 'capacity_consumption': 100},
        )
        ...
        self.assertEqual(
            self.chucky.installed_software, [{'name': 'PyCharm', 'memory_consumption': 50, 'capacity_consumption': 100}])
    ...
    def test_install_software_with_too_much_requirements_return_error_message(self):
        result = self.robot_with_softwares.install_software({'name': 'Cam', 'memory_consumption': 200, 'capacity_consumption': 400})
        self.assertEqual(result, f"Software 'Cam' cannot be installed on Indoor part.")
        self.assertEqual(self.robot_with_softwares.installed_software,
                         [{"name": "Pycharm", 'capacity_consumption': 20, 'memory_consumption': 40}]
                         )
